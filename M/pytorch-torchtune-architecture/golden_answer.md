...
Prefix tuning attaches `P_k, P_v in R^{prefix_len x num_kv_heads x head_dim}` per layer and concatenates them onto K/V before SDPA. **No `nn.Linear` is wrapped.** No residual to merge. The adapter lives inside (or next to) attention, not in place of a projection.
...
**Recommendation: (a) for training** (memory budget is known and small, the per-layer prefix is `prefix_len x num_kv_heads x head_dim` floats -- typically <1MB total), **(a) for inference** also (the prefix doesn't change between requests; caching it once is strictly better than recomputing).
